...
def test_compare_controllers(self):
    ...
    compliant_twist.set_parameter_value("linear_principle_damping", lpd, sr.ParameterType.DOUBLE)
    compliant_twist.set_parameter_value("linear_orthogonal_damping", lod, sr.ParameterType.DOUBLE)
    compliant_twist.set_parameter_value("angular_stiffness", ang_stiff, sr.ParameterType.DOUBLE)
    compliant_twist.set_parameter_value("angular_damping", ang_damping, sr.ParameterType.DOUBLE)
    ...
    dissipative.set_parameter_value("damping_eigenvalues", damping_eigenvalues, sr.ParameterType.VECTOR)
    ...
    np.testing.assert_almost_equal(compliant_command.get_torque(), impedance_command.get_torque())
    np.testing.assert_almost_equal(compliant_command.get_force(), dissipative_command.get_force())
...
class CartesianSpaceController(Node):
    ...
    def controller_compliant_twist(self, state):
        ...
        return self.ctrl.compute_command(
            twist, tmp_state, state.jacobian)
    ...
    def controller_velocity_impedance_simplified(self, state, desired_twist=None):
        ...
        if desired_twist is None:
            desired_twist = sr.CartesianTwist(self.ds_dissipative.evaluate(state.ee_state))
        ...
        torque = D[3:, 3:] @ (desired_twist.get_angular_velocity() - tmp_state.get_angular_velocity())
        print("\n")
        ...
        des_virt_state.set_angular_velocity(desired_virutal_twist.get_angular_velocity())
    ...
    def controller_velocity_impedance_with_integration(self, state, desired_twist=None):
        ...
        torque = K[3:, 3:] @ pos_error + D[3:, 3:] @ (des_virt_state.get_angular_velocity() -
                                                      tmp_state.get_angular_velocity())
        proportional = K[3:, 3:] @ des_virt_state.get_angular_velocity() * 0.5
        damping = D[3:, 3:] @ (des_virt_state.get_angular_velocity() - tmp_state.get_angular_velocity())
        ...
        return ctrl_impedance.compute_command(
            des_virt_state, tmp_state, state.jacobian)
    ...
    def controller_callback(self) -> None:
        ...
        dissipative_twist = sr.CartesianTwist(self.ds_dissipative.evaluate(state.ee_state))
