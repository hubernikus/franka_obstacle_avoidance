...
from franka_avoidance.human_optitrack_container import HumanTrackContainer
...
class NonlinearAvoidanceController(Node):
    ...
    def __init__(
        ...
        if is_simulation:
            ...
            self.ctrl.set_parameter_value(
                "linear_principle_damping", 25.0, sr.ParameterType.DOUBLE
            )
            self.ctrl.set_parameter_value(
                "linear_orthogonal_damping", 25.0, sr.ParameterType.DOUBLE
            )
            self.ctrl.set_parameter_value(
        ...
        self.avoider = MultiObstacleAvoider(obstacle=self.human_with_limbs)
    ...
    def publisher_trajectory_initial(
        ...
        for ii in range(it_max):
            ...
            conv_velocity = self.avoider.get_tangent_direction(
                position, velocity, linear_velocity
            )
    ...
    def controller_callback(self) -> None:
        ...
        # breakpoint()
        linear_velocity = self.rotation_projector.evaluate_convergence_around_obstacle(
            position, root_obstacle
        )
        ...
        # twist.set_linear_velocity(conv_velocity)
