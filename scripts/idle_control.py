...
def control_loop(robot, freq):
    ...
    ds.set_parameter_value(
        "gain", [0.0, 0.0, 0.0, 10.0, 10.0, 10.0], sr.ParameterType.DOUBLE_ARRAY
    )

    ctrl = create_cartesian_controller(CONTROLLER_TYPE.COMPLIANT_TWIST)
    ctrl.set_parameter_value("linear_principle_damping", 0.0, sr.ParameterType.DOUBLE)
    ctrl.set_parameter_value("linear_orthogonal_damping", 0.0, sr.ParameterType.DOUBLE)
    ctrl.set_parameter_value("angular_stiffness", 5.0, sr.ParameterType.DOUBLE)
    ctrl.set_parameter_value("angular_damping", 5.0, sr.ParameterType.DOUBLE)
    ...
    while not rospy.is_shutdown():
        ...
        if not target_set:
            ...
            command_torques = sr.JointTorques(
                ctrl.compute_command(twist, state.ee_state, state.jacobian)
            )
